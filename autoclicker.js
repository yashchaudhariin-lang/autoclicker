...
bridge.onMessage(function(msg, sender, sendResponse) {
  // TEMPORARY DIAGNOSTIC: log every incoming message to confirm START reaches us
  console.log("[AUTOCLICKER_BRIDGE] raw incoming message:", JSON.stringify(msg));

  if (msg.source !== "popup") {
    // Message not ours — explicitly return false so Chrome doesn't hold
    // the response port open. Same class of bug fixed earlier in
    // license.js/background.js — a bare return (undefined) causes
    // port-closed errors when multiple listeners exist.
    return false;
  }
  ...
  // Unrecognized action with popup source — return false to signal
